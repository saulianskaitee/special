...
parser.add_argument('-a', '--af-version', dest='af_version', default="AF2", choices=["AF2","AF3"], type=str, help="Specify the AlphaFold version used (AF2, AF3)")
...
# INFO mode - more informative (display all the messages)
# ERROR mode - only display the error messages
# WARRNING mode...
if args.verbose:
...
if args.af_version == "AF2":
    ...
    if args.models is None:
        ...
        #user specified the exact number of models to be processed
...
#AlphaFold3
    ...
    #parse json file with ranking scores
    #information about the ranking scores is stored in the "fold_2024_08_06_11_17_summary_confidences_0.json" files
    files = [fname for fname in os.listdir(args.af_dir) if re.search("summary_confidences", fname)]
    
    #make a dictionary of json files with pae values together with the ranking scores, sort the dictionary according the scores
    files_ranked = dict()
    for i, fname in enumerate(files):
        with open(os.path.join(args.af_dir, fname), 'r') as fname:
            this_data = json.load(fname)
            rank_score = this_data["ranking_score"]
            files_ranked[f"{dir}_full_data_{i}.json"] = rank_score

    #sort the dictionary so that the 1st file woulv have the highest rank score
    files_ranked = dict(sorted(files_ranked.items(), key=lambda item: item[1], reverse = True))

    #the ranking was found
    log.info(f"Found the following model ranking:")
    for i, fname in enumerate(files_ranked):
        log.info(f"{i}\t{fname}")
        #or
        #log.info(f"Model {i}:\t{files_ranked[fname]}")

    #dictionary of files into list sorted by the scores
    files_pae = [key for key in files_ranked]

    #check if provided number of models does not exceed the max number of models
    if args.models is None or len(files_pae) < args.models:
        log.info("All models will be processed")
        args.models = len(files_pae)
    elif args.models is not None and len(files_pae) > args.models:
        log.info(f"Only the top {args.models} ranking models will be considered")

    #gathering pae scores and plotting the graphs
    for i, file_name in enumerate(files_pae[:args.models]):
        log.info(f"processing {file_name} ({i+1} / {len(files_pae[:args.models])})...")
        ...
        try:
            ...
            log.error(f"Couldn't open {file_name} in the specified input directory; exiting...")
            exit(1)
        except json.JSONDecodeError:
            log.error(f"Couldn't parse {file_name}; is it in the right format? Exiting...")
            exit(1)
        except KeyError:
            log.error(f"{file_name} doesn't contain PAE information; Exiting...")
            exit(1)

        log.info(f"PAE matrix is {pae.shape[0]} x {pae.shape[1]}")

        # save as text file
        np.savetxt(f"pae_model_{i}.dat", pae, fmt=args.fmt)
        ...
        # generate ticks and their labels
        if args.consecutive_numbering:
            #why we are not just writting ticks = np.arange(1, pae.shape[0] + 1, args.tick_interval)?
            ticks = np.arange(0, pae.shape[0] + 1, args.tick_interval) - 1
            ticks[0] = 0
            tick_labels = ticks + 1
        else:
            try:
                structure = cif_parser.get_structure('structure', os.path.join(args.af_dir, f"_model_{i}.cif"))
            except IOError:
                log.error(f"Couldn't open ranked_{i}.pdb in the specified input directory; exiting...")
                exit(1)
            ...
            try:
                ...
                log.error(f"no model found in {args.af_dir}_model_{i}.cif")
                exit(1)
...
#AlphaFold2
    ...
    log.error(f"You need to specify which AlphaFold version (AF2 or AF3) is used; Exiting...")
    exit(1)
